...
run: async (client, message, args) => {
  ...
  if(!guildId || isNaN(guildId) || guildId.length > 18) return message.reply('Please provide guild ID!');
  let guild = client.guilds.cache.get(guildId);
  if(!guild) return message.channel.send(embed.setColor(colours.red).setDescription('The client isn\'t in that guild.'));
  let invitePossiblites = guild.channels.cache.filter(cha => cha.permissionsFor(guild.me).has('CREATE_INSTANT_INVITE'));
  if(!invitePossiblites) return message.channel.send(embed.setColor(colours.red).setDescription('Couldn\'t fetch a channel that allowed me to make an invite.'));
    ...
    invitePossiblites.random().createInvite()
      ...
      **[Invite](${`https://discordapp.com/invite/${invite.code})** || **Code: ${invite.code}**`}`));
        ...
        message.channel.send(embed.setColor(colours.red).setDescription(`couldn\'t make invite.`))
      }
